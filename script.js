...
// ============================================
// AUDIO — Game Clock Buzzer
// ============================================
function playGameBuzzer() {
  try {
...
function addTimeout(t) {
  if (G.tos[t] < MAXT) {
    ...
    // Game clock keeps running — do NOT pause

    // Pause shot clock during timeout
    G.ssec_running = false;
    updShot();
    ...
    // Auto start timeout clock
    if (G.to_iv) clearInterval(G.to_iv);
    toggleToClock();

    // Sync game clock in overlay
    updToGameClock();
...
function closeTimeout() {
  ...
  // Reset shot clock to 24s after timeout finishes
  resetShot(24);
}
...
function updClock() {
  ...
  if (G.gsec < 60 && G.gsec > 0) {
    clockText = (G.gsec).toFixed(1);
  } else {
    clockText = fmt(Math.ceil(G.gsec));
  }
  el.textContent = clockText;
  el.className = 'clk-num' + (G.gsec <= 30 ? ' danger' : G.gsec <= 60 ? ' warn' : '');

  // Also update game clock in timeout overlay if active
  if (G.to_active) {
    updToGameClock();
  }
}

// Update game clock display inside timeout overlay
function updToGameClock() {
  const el = document.getElementById('to-gameclock');
  if (!el) return;
  if (G.gsec < 60 && G.gsec > 0) {
  ...
  // Color states
  if (G.gsec <= 30) {
    el.className = 'to-gameclock danger';
  } else if (G.gsec <= 60) {
    el.className = 'to-gameclock warn';
  } else {
    el.className = 'to-gameclock';
  }
}
...
function toggleClock() {
  if (G.running) {
    ...
    G.iv = setInterval(() => {
      ...
      // Shot clock expired — game clock keeps running!
...
document.addEventListener('keydown', e => {
  ...
  // Bell shortcut is always available when not typing
  if (k === 'b') { playTimeoutBell(); return; }
